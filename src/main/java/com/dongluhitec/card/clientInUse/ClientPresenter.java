        ...
        public void messageReceived(final IoSession session, Object message)
            ...
            if (checkSubpackage.startsWith("swipeCard", 21)) {
                Display.getDefault().asyncExec(new Runnable() {
                    ...
                    public void run() {
                        String responseSwipeCardInfo = "";
                        if (!DEV_MODE) {
                            ClientPresenterClient.CardSN(dom.getRootElement()
                                    .elementTextTrim("cardSerialNumber"));
                            responseSwipeCardInfo = HardwareUtil
                                    .responseSwipeCardInfo(session, dom,
                                            clientUI.Ok2Xml());                            
//                            clientUI.println("swipeCard:"
//                                    + dom.getRootElement().getPath()
//                                    + dom.getRootElement().elementTextTrim(
//                                            "cardSerialNumber"));
                        } else {
                            responseSwipeCardInfo = HardwareUtil
                                    .responseSwipeCardInfo(session, dom,
